...
void main(string[] args) {
    Window myWindow = exampleInit(nstring("Texture Rendering"), 640, 480, args);
    ...
    // Texture & Sampler
    NuvkTexture adaTexture = device.createFromPNG(ada);
    NuvkTextureView adaView = adaTexture.createTextureView(NuvkTextureViewDescriptor(
    ...
    while(!myWindow.isCloseRequested()) {
        ...
        if (auto cmdbuffer = queue.nextCommandBuffer()) {
            if (NuvkTextureView nextImage = swapchain.getNext()) {
                ...
                uniform0.mvp = (
                    mat4.orthographic01(0, fbSize.x, 0, fbSize.y, 0, 1000)
                ).transposed();
                ...
                if (NuvkRenderEncoder renderPass = cmdbuffer.beginRenderPass(renderPassDescriptor)) {
                    ...
                    renderPass.setFragmentTexture(adaView, 0);
