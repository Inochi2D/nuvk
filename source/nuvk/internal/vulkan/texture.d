...
public:
    ...
    */
    ...
    VkImageMemoryBarrier createTransition(NuvkTextureLayout newLayout, NuvkSyncDirection direction) {
        ...
        return VkImageMemoryBarrier(
            ...
            subresourceRange: VkImageSubresourceRange(
                aspectMask: this.getFormat().toVkAspectFlags(),
                baseMipLevel: 0,
                levelCount: this.getMipCount(),
                baseArrayLayer: 0,
    ...
    VkPipelineStageFlagBits[2] createTransitionStages(NuvkSyncDirection direction) {
        return [VK_PIPELINE_STAGE_NONE, VK_PIPELINE_STAGE_NONE];
    }
}
...
private:
    ...
    void createTextureView(NuvkTexture texture) {
        ...
        imageViewCreateInfo.image = texture.getHandle!VkImage();
...
VkCompareOp toVkCompareOp(NuvkCompareOp compOp) @nogc {
    final switch(compOp) {

        case NuvkCompareOp.never:
            return VK_COMPARE_OP_NEVER;

        case NuvkCompareOp.less:
            return VK_COMPARE_OP_LESS;

        case NuvkCompareOp.equal:
            return VK_COMPARE_OP_EQUAL;

        case NuvkCompareOp.lessEqual:
            return VK_COMPARE_OP_LESS_OR_EQUAL;

        case NuvkCompareOp.greater:
            return VK_COMPARE_OP_GREATER;

        case NuvkCompareOp.greaterEqual:
            return VK_COMPARE_OP_GREATER_OR_EQUAL;

        case NuvkCompareOp.notEqual:
            return VK_COMPARE_OP_NOT_EQUAL;

        case NuvkCompareOp.always:
...
/**
    Converts a Nuvk texture format + sync direction to a vulkan access flag.
*/
VkAccessFlagBits toVkAccessFlags(NuvkTextureFormat format, NuvkSyncDirection direction) @nogc {
    VkAccessFlagBits bits = cast(VkAccessFlagBits)0;
    switch(format) {
        case NuvkTextureFormat.depthStencil:
            if (direction & NuvkSyncDirection.read) 
                bits |= VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT;
            if (direction & NuvkSyncDirection.write) 
                bits |= VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
            return bits;
        default:
            if (direction & NuvkSyncDirection.read) 
                bits |= VK_ACCESS_COLOR_ATTACHMENT_READ_BIT;
            if (direction & NuvkSyncDirection.write) 
